	...
	applyStateUpdate(state: MainSimulationState): void {
		...
		so.mapLocations.splice(
			so.mapLocations.findIndex(
				f => f.name === this.feature.name && f.ownerId === this.feature.ownerId,
			),
			1,
		);
	}
...
export class AddActorLocalEvent extends LocalEventBase {
	constructor(
		...
		private travelTime: SimDuration,
	) {
	...
	applyStateUpdate(state: MainSimulationState): void {
		...
		localEventManager.queueLocalEvent(
			new ResourceGroupBinding(this.parentEventId, now + this.travelTime, actor.Uid),
		);
	}
}
...
export class TransferResourcesLocalEvent extends LocalEventBase {
	...
	applyStateUpdate(state: MainSimulationState): void {
		ResourceState.transferResourcesBetweenActors(
			state,
			this.senderActor,
			this.receiverActor,
			this.sentResources,
...
export class ResourcesArrivalLocalEvent extends LocalEventBase {
	...
	applyStateUpdate(state: MainSimulationState): void {
		...
		if (actors.length === 1) {
			// AL case create group if not existant
			resourceGroup = getOrCreateResourceGroup(state, actors[0].Uid);
		} else {
			// multiple actors present but some might be traveling
			resourceGroup = actors
				.map(a => state.getResourceGroupByActorId(a.Uid))
				.find(rg => rg !== undefined);
			if (!resourceGroup) {
...
export class ResourcesReleaseLocalEvent extends LocalEventBase {
	...
	applyStateUpdate(state: MainSimulationState): void {
		ResourceState.releaseResourcesFromTask(
			state,
			this.taskId,
			this.actorId,
			this.resourceType,
