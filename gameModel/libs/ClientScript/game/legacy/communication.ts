...
export function processDirectMessageEvent(
  event: FullEvent<DirectCommunicationEvent>,
  senderId: string
) {
...
export function getChannelChoices(radioId: number): { label: string; value: string }[] {
  ...
  if (r) {
    return r.availableChannels.map(v => {
...
export function processRadioCommunication(event: FullEvent<RadioCommunicationEvent>): void {
  ...
  if (!radio) {
    commLogger.error(
      'Cannot handle message, radio with id ' + event.payload.senderRadioId + ' it does not exist'
    );
  } else {
  ...
  if (myPhone) {
    ...
    if (phoneMessages[myPhone.phoneId]) {
      ...
      return filtered.map(m => {
        return `${m.time} : ${m.payload.message}`;
      });
    } else {
      return [noMessages];
    }
...
export function emitPhoneMessageEvent(
