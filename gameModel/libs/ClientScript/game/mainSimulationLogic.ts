...
function initActionTemplates(): Record<string, ActionTemplateBase> {
	...
	const placeAccessRegress = new SelectMapObjectTemplate(
		'define-accreg-title',
		'define-accreg-desc',
		TimeSliceDuration * 3,
	...
	const placePMA = new SelectMapObjectTemplate(
		'define-PMA-title',
		'define-PMA-desc',
		TimeSliceDuration * 4,
		'define-PMA-feedback',
		{
			featureSelection: {
				layerId: 'buildings',
				featureKey: '@id',
				featureIds: ['way/301355984', 'way/82683752', 'way/179543646'],
			},
		},
	);
	const placePC = new SelectMapObjectTemplate(
		'define-PC-title',
		'define-PC-desc',
		TimeSliceDuration * 2,
		'define-PC-feedback',
	...
	const placeNest = new SelectMapObjectTemplate(
		'define-Nest-title',
		'define-Nest-desc',
		TimeSliceDuration * 3,
	...
	const sendResources = new SendResourcesToActorActionTemplate(
		'send-resources-title',
		'send-resources-desc',
		TimeSliceDuration,
		'send-resources-feedback',
	);

	const assignTaskToResources = new AssignTaskToResourcesActionTemplate(
		'assign-task-title',
		'assign-task-desc',
		TimeSliceDuration,
		'assign-task-feedback',
	);
	const releaseResourcesFromTask = new ReleaseResourcesFromTaskActionTemplate(
		'release-task-title',
		'release-task-desc',
		TimeSliceDuration,
		'release-task-feedback',
	);
...
function processEvent(event: FullEvent<TimedEventPayload>) {
	...
	switch (event.payload.type) {
		...
		case 'ResourceAllocationEvent': {
			const newLocalEvent = TaskLogic.createResourceAllocationLocalEvent(
				event as FullEvent<ResourceAllocationEvent>,
		...
		case 'ResourceReleaseEvent': {
			const newLocalEvent = TaskLogic.createResourceReleaseLocalEvent(
				event as FullEvent<ResourceReleaseEvent>,
				currentSimulationState,
			);
			if (newLocalEvent != null) {
				localEventManager.queueLocalEvent(newLocalEvent);
...
export async function buildAndLaunchResourceAllocation(
	taskId: TaskId,
	selectedActor: ActorId,
...
export async function buildAndLaunchResourceRelease(
	taskId: TaskId,
	selectedActor: ActorId,
	resourceType: ResourceType,
	nbResources: number,
