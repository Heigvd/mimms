...
export function getBuildingsLayer(feature: any, resolution: number) {
  ...
  const selectionFeatures = mapLocations
    .filter(feature => feature.geometryType === 'Select')
    .flatMap(feature => (feature as SelectFeature).featureIds);
  ...
  if (selectionFeatures.includes(feature.get('@id'))) {
    const selectionFeature = mapLocations.find(
      mapLocation =>
        mapLocation.geometryType === 'Select' && mapLocation.featureIds === feature.get('@id'),
    );
    // Check if the PMA is available or not
    buildingStyle.fill!.color =
      selectionFeature!.startTimeSec! + selectionFeature!.durationTimeSec! > getSimTime()
        ? '#575FCF80'
        : '#575FCF';
