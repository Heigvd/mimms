 ...
 */
export function createResourceAllocationLocalEvent(
	globalEvent: FullEvent<ResourceAllocationEvent>,
	state: MainSimulationState,
): LocalEventBase | undefined {
	taskLogger.info('resources allocation asked ' + JSON.stringify(globalEvent.payload));

	const effectiveNb = findEffectiveNbResourcesToAllocate(
		state,
		globalEvent.payload.taskId,
		globalEvent.payload.actorId,
		globalEvent.payload.resourceType,
		globalEvent.payload.nbResources,
	);

	taskLogger.debug('resources allocation given ' + effectiveNb);

	return new ResourcesAllocationLocalEvent(
		globalEvent.id,
		globalEvent.payload.triggerTime,
		globalEvent.payload.taskId,
		globalEvent.payload.actorId,
		globalEvent.payload.resourceType,
		effectiveNb,
 ...
 */
function findEffectiveNbResourcesToAllocate(
	...
	if (TaskState.isTaskAlive(state, taskId)) {
		...
		const taskNbStillMissingResources = TaskState.getNbResourcesStillUsefulForTask(
			state,
			taskId,
			type,
		);
 ...
 */
export function createResourceReleaseLocalEvent(
	globalEvent: FullEvent<ResourceReleaseEvent>,
	state: MainSimulationState,
): LocalEventBase | undefined {
	taskLogger.debug('resources release asked ' + globalEvent.payload.nbResources);

	const effectiveNb = findEffectiveNbResourcesToRelease(
		state,
		globalEvent.payload.taskId,
		globalEvent.payload.actorId,
		globalEvent.payload.resourceType,
		Math.abs(globalEvent.payload.nbResources),
	);

	taskLogger.debug('resources release given ' + effectiveNb);

	return new ResourcesReleaseLocalEvent(
		globalEvent.id,
		globalEvent.payload.triggerTime,
		globalEvent.payload.taskId,
		globalEvent.payload.actorId,
 ...
 */
function findEffectiveNbResourcesToRelease(
	state: Readonly<MainSimulationState>,
	taskId: TaskId,
	actorId: ActorId,
	type: ResourceType,
	...
	const nbAvailable: number = ResourceState.getResourcesAllocatedToTaskForActor(
		state,
		taskId,
		actorId,
		type,
