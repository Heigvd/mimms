...
export function createResourceAllocationLocalEvent(globalEvent: FullEvent<ResourceAllocationEvent>, state: MainSimulationState): LocalEventBase | undefined{
  ...
  let effectiveNb = findEffectiveNbResourcesToAllocate(state,
    globalEvent.payload.taskId, globalEvent.payload.actorId, globalEvent.payload.kind, globalEvent.payload.nbResources);

  taskLogger.debug("resources allocation given " + effectiveNb);

  return new ResourcesAllocationLocalEvent(globalEvent.id, globalEvent.payload.triggerTime, globalEvent.payload.taskId, globalEvent.payload.actorId, globalEvent.payload.kind, effectiveNb);
}
...
function findEffectiveNbResourcesToAllocate(state: Readonly<MainSimulationState>,
  taskId: TaskId, actorId: ActorId, kind: ResourceKind, nbRequested: number): number {

    if (TaskState.isTaskAlive(state, taskId)) {
      const nbAvailable: number = ResourceState.getResourcesAvailable(state, actorId, kind).length;
      const taskNbStillMissingResources = TaskState.getNbResourcesStillUsefulForTask(state, taskId, kind);
...
export function createResourceReleaseLocalEvent(globalEvent: FullEvent<ResourceReleaseEvent>, state: MainSimulationState): LocalEventBase | undefined {
  ...
  let effectiveNb = findEffectiveNbResourcesToRelease(state,
    globalEvent.payload.taskId, globalEvent.payload.actorId, globalEvent.payload.kind, Math.abs(globalEvent.payload.nbResources));

  taskLogger.debug("resources release given " + effectiveNb);

  return new ResourcesReleaseLocalEvent(globalEvent.id, globalEvent.payload.triggerTime, globalEvent.payload.taskId, globalEvent.payload.actorId, globalEvent.payload.kind, effectiveNb);
}

/**
 * @returns The number of resources owned by the given actor that can be released from the given task.
 * The restrictions are : how many are requested, how many resources resources are allocated to the task.
 */
function findEffectiveNbResourcesToRelease(state: Readonly<MainSimulationState>,
  taskId: TaskId, actorId: ActorId, kind: ResourceKind, nbRequested: number): number {

    const nbAvailable: number = ResourceState.getResourcesAllocatedToTaskForActor(state, taskId, actorId, kind).length;
