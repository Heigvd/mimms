...
export const obstacleExtents = Helpers.useRef(
	'recursiveExtents',
	{} as Record<ObstacleType, DiscreteExtent[]>,
);
...
export function tryLoadExtentAsync(
	mapId: string,
	extentState: ExtentState,
	setState: ExtentStateSetter,
): ExtentState {
		...
		case 'NOT LOADED': {
			...
			req
				.then(t => {
					...
					extentState.extent = OpenLayer.transformExtent(
						ext,
						gpsProjection,
...
export class DiscreteExtent {
	...
	public split(n: number): DiscreteExtent[] {
		...
		if (deltaX < 2 && deltaY < 2) throw new Error('Cannot split extent');
 ...
 */
export function updateObstacleMatrixWithLayer(
	layer: any,
	map: any,
	mapId: string,
	obstacleValue: ObstacleType,
		...
		vecSource.forEachFeatureIntersectingExtent(e, (f: any) => {
			...
			if (shape) {
				...
				if (g.getType() === 'MultiPolygon') {
					...
					// => remove them
					cutShape.geometry.coordinates = cutShape.geometry.coordinates.filter(
						(c: any) => c.length > 0,
					);
				}
...
function internalRecursiveFill(
	grid: number[][],
	extent: DiscreteExtent,
	vecSrc: any,
	oLayer: ObstacleLayer,
