...
import {
  getNextNonTransportedPatientByPriority,
...
export class PorterTask extends DefaultTask {
  ...
  private isAlreadyGroupMember(resourceId: number): boolean {
    return (
      this.resourcesGroups.find(group =>
        group.resources.find(groupedResourceId => groupedResourceId === resourceId),
      ) !== undefined
    );
  }
  ...
  private getAssignedToResourcesGroupPatientIds(): string[] {
    return this.resourcesGroups
      .filter(resourceGroup => resourceGroup.transportingPatientId !== undefined)
  ...
  protected dispatchInProgressEvents(state: Readonly<MainSimulationState>, timeJump: number): void {
    ...
    taskLogger.info(
      'Patients not transported before action: ' + getNonTransportedPatientsSize(state),
    );
    ...
    this.cleanupUnallocatedResourcesAndGroups(
      ResourceState.getAllocatedResourcesAnyKind(state, this.Uid),
    );
    ...
    this.resourcesGroups.map(resourceGroup => {
      const assignedToResourcesGroupPatients: string[] =
        this.getAssignedToResourcesGroupPatientIds();
      let nextPatient;
      if (resourceGroup.transportingPatientId)
        nextPatient = getPatient(state, resourceGroup.transportingPatientId);
      else
        nextPatient = getNextNonTransportedPatientByPriority(
          state,
          this.instructedToMovePatients.concat(assignedToResourcesGroupPatients),
        );
      ...
      if (nextPatient) {
        if (
          ...
          if (resourceGroup.cumulatedUnusedTime + timeJump >= this.TIME_REQUIRED_FOR_TRANSPORT) {
            resourceGroup.cumulatedUnusedTime =
              resourceGroup.cumulatedUnusedTime + timeJump - this.TIME_REQUIRED_FOR_TRANSPORT;
          ...
          localEventManager.queueLocalEvent(
            new PatientMovedLocalEvent(
              0,
              state.getSimTime() + this.TIME_REQUIRED_FOR_SELF_TRANSPORT,
              this.Uid,
    ...
    taskLogger.info(
      'Patients not transported after action: ' + getNonTransportedPatientsSize(state),
    );
      ...
      state.getAllActors().forEach(actor => {
        localEventManager.queueLocalEvent(
          new AddRadioMessageLocalEvent(
            ...
            actor!.Uid,
            'resources',
            this.feedbackAtEnd || 'TODO add task feedback',
          ),
        );
      });
    }
