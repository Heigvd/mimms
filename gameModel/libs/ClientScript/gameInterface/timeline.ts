...
function createGridTimes(maxTime: number, currentTime: number): string {
  ...
  for (let i = 0; i < steps; i++) {
    ...
    output += createGridSegment(
      ...
      '',
      'marker-time',
      `<div class="${isCurrentTime ? 'time current' : 'time'}" ${
        isCurrentTime ? `id="current-time"` : ''
    ...
    output += createGridSegment(
      ...
      '',
      'marker',
      `<div class="${isCurrentTime ? 'marker-line current' : 'marker-line'}"></div>`
    );
    timer += 60;
  ...
  while (actionIndex < actions.length) {
    ...
    if (timer === actions[actionIndex]!.startTime) {
      ...
      const animationClassName =
        current && actions[actionIndex]!.startTime >= simTime ? 'timeline-animation' : '';
      let timelineSize = '';
      if (actionDuration === 60) {
        timelineSize = 'col-size-small';
      }

      const className = activeclassName.concat(
        ' ',
        futureClassName,
        ' ',
        timelineSize,
